...
import ForgotPasswordDialog from "../components/auth/ForgotPasswordDialog"; // yo'lni mosla
...
import {
  ...
  IconButton, FormControlLabel, Checkbox, Button, Divider, Stack,Container
} from "@mui/material";
...
// === Colors: o'zingizni sozlang
const GRADIENT_FROM = "#6a5cff";
const GRADIENT_TO   = "#7bd3ff";

// Firebase (sizning config faylingizdan import qiling)
import {
  auth,
  googleProvider,
  githubProvider,
} from "../firebase"; // <- o'zingizdagi yo'l
...
export default function Login() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // navigate("/"); // xohlasangiz redirect qiling
    } catch (err: any) {
      const code = err?.code || "";
      let msg = "Kirishda xatolik";
      if (code === "auth/user-not-found") msg = "Foydalanuvchi topilmadi";
      else if (code === "auth/wrong-password" || code === "auth/invalid-credential") msg = "Parol yoki email noto‘g‘ri";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
    ...
    >
      <Card
        ...
        sx={{
          ...
          bgcolor: (t) =>
            t.palette.mode === "dark" ? "rgba(18,18,18,.6)" : "rgba(255,255,255,.7)",
          border: "1px solid",
          borderColor: "divider",
        }}
      >
        <CardContent sx={{ p: 4 }}>
          ...
          <Box component="form" onSubmit={onSubmit}>
            ...
            >
              <FormControlLabel
                ...
                label="Remember me"
              />
            <Button variant="text" size="small" onClick={() => setFpOpen(true)}>
              Forgot password?
            </Button>
            </Stack>
          ...
          >
            ...
            <Button size="small" component={Link} to='/signup' variant="text" sx={{ textTransform: "none", px: .5 }}>
              Sign up
            </Button>
          </Typography>
        </CardContent>
